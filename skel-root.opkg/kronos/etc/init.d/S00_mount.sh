...
## mout rootX
ROOT_EXT=root1
ROOT_EXT_DEV=$(grep -i $ROOT_EXT /proc/mtd | cut -f 0 -s -d :)
if [ -d $ROOT_EXT_DEV ]; then
	echo no $ROOT_EXT partition found
else
	ROOT_EXT_MOUNT=/ext
	[ ! -d $ROOT_EXT_MOUNT ] && /bin/mkdir -p $ROOT_EXT_MOUNT
	ROOT_EXT_BLOCK=$(grep -i $ROOT_EXT /proc/mtd | cut -b 4)
	echo try to mount /dev/mtdblock$ROOT_EXT_BLOCK to $ROOT_EXT_MOUNT
	/bin/mount -t jffs2 /dev/mtdblock$ROOT_EXT_BLOCK $ROOT_EXT_MOUNT
fi

## format drive if exists flagfile
#[ -e /var/etc/.make-diskpart.sh ] && [ -x /var/sbin/make-diskpart.sh ] && /var/sbin/make-diskpart.sh
